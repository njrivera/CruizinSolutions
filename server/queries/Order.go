...
const (
	CreateOrder     = "INSERT INTO orders (date, cid, vid, odometer, comments, subtotal, tax, total, payment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
	...
	GetOrders       = "SELECT O.ordernum, O.date, V.vid, V.year, V.make, V.model, O.odometer, O.comments, O.subtotal, O.tax, O.total, O.payment FROM orders O, vehicles V WHERE cid = ? AND V.vid = O.vid"
